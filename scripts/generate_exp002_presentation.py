...
def build_presentation():
    ...
    add_multiline_textbox(slide, 6200000, 1200000, 5500000, 5000000, [
        ...
        ("", False),
        ("島の南東部", True),
        # 【2026-08-23 訂正により無効】この「6,521m² が『北木の桂林』に相当か」という
        # 記述は、報告書 §5.3 の訂正（OpenStreetMap 照合にもとづく）により否定されて
        # いる。訂正後の同定では「北木の桂林」は島北部の最大水域 7,826m² に対応し、
        # この南東部のポリゴンの周囲約700m以内には該当する OSM 地物が無い。
        # 本行は既に配布した成果物（exp002_kitagi_quarry_water_detection_presentation.pptx）
        # を再現するスクリプトであるため文言は変更せず、pptx も再生成しない。
        ("2番目に大きい水域 6,521m²。\n「北木の桂林」に相当か。", False),
        ("", False),
        ("島の中央部", True),
        ("中規模の水域が散在。", False),
