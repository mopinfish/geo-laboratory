...
| `basemap_kitagi_carto_positron.png` | 取得元: CARTO Positron ラベルなし `https://a.basemaps.cartocdn.com/light_nolabels/{z}/{x}/{y}.png`（contextily の `cx.providers.CartoDB.PositronNoLabels` と同一タイル、ズーム17、範囲 west 133.5135 / south 34.3645 / east 133.5630 / north 34.4045、取得日 2026-08-24）。取得は `exp002_kitagi_foss4g2026_figures.py --fetch-basemap` で一度だけ行い、以後はこの追跡ファイルを読むのみ（下記2.4参照） | `acf78b8e799e9096a06ec69c4dbed40955969f5b71ff10e278dcb1c1631ee6e1` | （直接配置せず）P6・P7パネル(c)・P8 の背景地図 |
...
| `p06_clusters_map.png` | 生成元: `docs/presentations/exp002_kitagi_foss4g2026_figures.py` の `make_p06_clusters_map()`（入力: 公開GeoJSON（SHA256は下記2.1参照）＋ `basemap_kitagi_carto_positron.png`） | `568fb1bf00b3d928dcaf395feeb4d679eec924e51c9cbc54170ddb0250b5243d` | S6 |
| `p07_three_scales.png` | 生成元: 同スクリプトの `make_p07_three_scales()`（入力: `choba_lake_3.jpg`・`aerial_quarry_pond.jpg`・公開GeoJSON＋`basemap_kitagi_carto_positron.png`。英語ラベルのみの発表専用合成図） | `e86e95f1becb17781e4dc3ed29dd9527584fa15b85bb45ff4afa80432a5e129f` | S7 |
| `p08_visit_anchors_map.png` | 生成元: 同スクリプトの `make_p08_visit_anchors_map()`（入力: 公開GeoJSON＋訪問記図4の座標確認済み4地点＋`basemap_kitagi_carto_positron.png`） | `cd76298d6fbc0577721719700bf0f68c00f990817e04b39ee553c9f3b11fffbe` | S8 |
...
| 範囲（west, south, east, north） | 133.5135, 34.3645, 133.5630, 34.4045（EPSG:4326。図版の地図軸 `MAP_BBOX` と厳密に一致。2026-08-24 のレイアウト是正で島の陸域の実測 bbox + 約 230 m の余白へ広げ、同じ範囲で取り直した） |
| 保存画素数 | 4,614 × 4,518 px（地上分解能約 1.0 m/px。出力1pxあたり約2.5px） |
| 取得日 | 2026-08-24 |
| 取得タイル数 | 342 枚（全球を覆うプロバイダのため欠測なし。404 は取得コードで即座に失敗させる） |
| 追跡ファイル | `docs/presentations/images/basemap_kitagi_carto_positron.png`（1.00 MB） |
| 帰属表示 | `Basemap: © OpenStreetMap contributors, © CARTO`（P6・P8 は native 22pt、P7パネル(c) は native 11.5pt。**地図枠の下**に1行のキャプションとして焼き込む。実効 11.1〜11.4pt でフッター階層と同じ） |

**他のタイルを使わなかった理由**

- 地理院タイル英語版 `xyz/english/`: **ズームレベル 5〜11 しか存在しない**（z12 以上は 404。
  2026-08-24 に実測して確認）。z11 の地上分解能は約 76 m/px で、地図範囲（幅 5,510 m）は
  わずか 72 px にしかならない。出力図版の地図軸は約 1,850 px 幅であり、「出力1pxあたり2px以上」
...
- **採用: CARTO Positron ラベルなし（`light_nolabels`）**。ラベルを一切含まないので
  ...
  ベクタ由来のフラットなラスタなので実写と違い PNG が十分に圧縮される（1.00 MB）。
...
| 帰属表示はフッター階層、置き場所は地図枠の下 | 図版内の帰属表示（`Basemap: © OpenStreetMap contributors, © CARTO`）は本文でも主題ラベルでもないため、15pt下限ではなくフッター階層（実効11〜12pt）に従う。S11 フッターが基図の帰属表示を 11pt で置いている前例に合わせる。置き場所は**地図枠の下**（1行のキャプション）とし、地図面には重ねない。2026-08-24 の第1波・是正では地図軸の内側・左上に白枠付きの箱として置いていたが、32pt級の図版では箱が大きく島の北西部を覆っていたため同日のレイアウト是正で外へ出した（軸の外に置いても図版の実寸高さは figsize が決めるため実効ptは落ちない。9章で実測） | 内容契約「実装時のハードゲート」節、`exp002_kitagi_foss4g2026_figures.py` の `add_basemap_credit()` / `assert_credit_outside_map()` |
| 検出地図3枚は1つの描画範囲を共有する | P6・S7パネル(c)・P8 は同一の `MAP_BBOX`（`133.5135/34.3645/133.5630/34.4045`）を使い、聴衆が3枚のスライドで同じ島の輪郭を見るようにする。範囲は島の陸域の実測 bbox（`133.51616/34.36661/133.56019/34.40224`、CARTO Positron z14 の陸海二値化＋連結成分で実測）に約 230 m の余白を足した値。解析側の `KITAGI_BBOX = [133.515, 34.350, 133.570, 34.400]` は使わない（南に約1.8kmの空虚な海が入り、かつ島の北端 34.40224 を切り落とす） | `exp002_kitagi_foss4g2026_figures.py` の `MAP_BBOX` / `ISLAND_BBOX_MEASURED` / `assert_map_bbox_frames_island()`、内容契約 S6・S7・S8 の Visual 注記 |
| S8 の訪問4地点は番号＋対応表で示す | 地図面には通し番号 1〜4（北→南）だけを短いリーダー線で置き、番号と地名の対応表は地図の下に2行×2列で示す。32pt のラベル箱は地図の縮尺で 1.2〜2.3 km に相当し、互いに 145〜470 m しか離れていない4地点の近くに地名を並べることは幾何学的に不可能である（従来は島の東側の海域に4枚を縦積みし、リーダー線を地図の上に長く走らせていた）。番号の位置は「軸の内側」「番号どうしが重ならない」「検出ポリゴンを覆わない」の3条件を満たす候補を機械探索して決め、3条件は生成時に assert で検査する | `exp002_kitagi_foss4g2026_figures.py` の `VISIT_ANCHOR_NUMBERS` / `VISIT_KEY_ROWS` / `P08_NUMBER_OFFSETS_M` / `assert_labels_clear_of_polygons()`、内容契約 S8 Visual 注記 |
| S7 の3パネルは表示寸法を揃える | 共通の縦横比はパネル(c) の地図範囲の比（≈1.02）とし、写真2枚を上下クロップだけで合わせる（引き伸ばしはしない）。地図の比は島の形と共有描画範囲で決まって縮めようがないのに対し、写真は上下を切ればどの比にも合わせられるため。3パネルの表示寸法が同一であることを実測 assert で固定した（DESIGN_GUIDE §4.3） | `exp002_kitagi_foss4g2026_figures.py` の `map_axes_ar()` / `make_p07_three_scales()`、内容契約 S7 Visual 注記 |
...
#### 6.3.1 語数・文字数と算出秒数（英語145 wpm換算／日本語300〜350字/分）

**英語の語数の算定方法**: 検査スクリプト `validate_exp002_kitagi_foss4g2026_presentation.py` の `count_en_words()` を用いた。これは正規表現 `[A-Za-z][A-Za-z'’-]*` による語トークナイザであり、アルファベットで始まる語のみを数え、`145` のような数字のみのトークンは数えない（数値の初出を綴り字で発話する方針と整合させるため）。対象は各スライドの `**EN (spoken)**` ブロックのみである（切り出しは最初の日本語マーカーで止まるため、日本語ブロック中の ASCII 語は数えない）。本節の語数・秒数はすべてこの方法による値であり、検査スクリプトの語数ゲート（割当語数の±25%）が用いる数え方と同一である。

比較のため、同じEN本文を単純な空白分割（`str.split()`、数字トークンも1語として数える）で数えると合計 2,528 語（本節の 2,511 語より17語多い）になる。数え方によって合計は数十語ずれるため、本節はどの数え方によるかを明示している。以下は `count_en_words()` の値である。

**日本語の文字数の算定方法**: 同スクリプトの `count_ja_chars()` を用いた。これは各スライドの `**JA（訳・読み上げ可）**` ブロック本文（マーカー行を除く）から空白・改行（正規表現 `\s`）を取り除いた文字数であり、句読点は数に含める。非発話ブロック（`**JA（補足・読み上げない）**`）は数えない。目安は割当秒数を **300〜350字/分**で換算した範囲であり、検査スクリプトはこの範囲を日本語原稿の分量ゲートとして用いる。

| S | 内容契約の時間割 | 割当語数（145wpm換算） | EN語数（算出） | 算出秒数（語数÷145×60） | 割当語数からの差 | JA想定範囲（300〜350字/分） | JA原稿文字数（算出） | JA字/分 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| 1 | 0:35 | 85 | 81 | 33.5秒 | −4.7% | 175〜204 | 183 | 314 |
| 2 | 1:40 | 242 | 229 | 94.8秒 | −5.4% | 500〜583 | 547 | 328 |
| 3 | 1:30（3–4で3:00） | 218 | 219 | 90.6秒 | +0.5% | 450〜525 | 486 | 324 |
| 4 | 1:30（3–4で3:00） | 218 | 220 | 91.0秒 | +0.9% | 450〜525 | 514 | 343 |
| 5 | 1:50 | 266 | 276 | 114.2秒 | +3.8% | 550〜642 | 642 | 350 |
| 6 | 2:30 | 362 | 357 | 147.7秒 | −1.4% | 750〜875 | 864 | 346 |
| 7 | 1:10 | 169 | 162 | 67.0秒 | −4.1% | 350〜408 | 366 | 314 |
| 8 | 1:30 | 218 | 221 | 91.4秒 | +1.4% | 450〜525 | 521 | 347 |
| 9 | 1:50 | 266 | 277 | 114.6秒 | +4.1% | 550〜642 | 630 | 344 |
| 10 | 1:20 | 193 | 186 | 77.0秒 | −3.6% | 400〜467 | 465 | 349 |
| 11 | 1:10（11–12で2:05） | 169 | 154 | 63.7秒 | −8.9% | 350〜408 | 408 | 350 |
| 12 | 0:55（11–12で2:05） | 133 | 129 | 53.4秒 | −3.0% | 275〜321 | 309 | 337 |
| **合計** | **17:30（1,050秒）** | **2,539** | **2,511** | **1,039.0秒（17:19）** | **−1.1%** | **5,250〜6,125** | **5,935** | **339** |

いずれのスライドも検査スクリプトの許容幅（英語は割当語数の±25%、日本語は300〜350字/分の範囲）以内であり、`validate_...py` は「OK: 1066 checks passed」で通過している（6.4節・9.5節）。S11 以外の11枚は割当語数の±5%以内に収まっている（S11 は −8.9%。口頭で挙げるライブラリ名を6つから3つへ削り、残りはフッターの投影に委ねた結果であり、投影面は変更していない。不足分は間として使える）。**S6単体は算出357語・147.7秒（2:28）で、ハードゲートの2:30（150秒）に対して2.3秒の余裕がある**（必須発話6項目は全て維持している）。全体の算出合計17:19は目標17:30に対して−11秒（−1.1%）で、20分枠（質疑5分・入替5分を除く発表20分）に対しては十分な余裕がある。**いずれの秒数も145 wpm換算の算出値であり、声に出して計測した実測値ではない。**

日本語原稿の分量は各スライド単位で 300〜350字/分の範囲に収めてあるが、実際に読む速度によって合計は動く。全12枚の合計5,935字は、350字/分で読めば1,017秒（16:57）、325字/分で1,096秒（18:16）、300字/分では1,187秒（19:47）に相当する。**日本語原稿は練習・自己確認・万一の代替のためのものであり、本番の通し時間の根拠は英語側である。** 日本語で通す場合は上限寄り（およそ340字/分以上）の速度が必要になる。
...
結果: `OK: 1066 checks passed`（2026-08-24 の修正ウェーブで検査群を両デッキへパラメータ化し、図中文字の実効サイズ検査を追加したため 466 → 949。スピーカーノートを3ブロック構成へ改め、日本語発話原稿の文字数ゲート・マーカー混入検査・全スライドに対する `[UPDATE AFTER 2026-08-31]` の発話ブロック禁止検査を追加したため 949 → 1064。同日のレイアウト是正で `p08_visit_anchors_map.png` の宣言要素が1つ増え、両デッキぶんで 1064 → 1066）。上記4コマンドの実行後に `git status --short` を確認したところ、差分は無かった（生成物は実行時刻に依存せずバイト単位で再現される）。すなわち `docs/presentations/` 配下の全生成物（PPTX 2種・図版5点）は再実行前と再実行後でバイト単位で一致した。
...
> **本章は 2026-08-24 の調整ウェーブ時点の成果物に対する記録である。** 同日のレイアウト是正
> （9章）で P6・P7・P8 の寸法・帰属表示の置き場所・S8 の訪問地点の示し方が変わったため、
> 本章 8.1〜8.4 の数値と目視所見のうち P6・P7・P8 に関わるものは**9章が差し替える**。
...
## 9. 2026-08-24 レイアウト是正ウェーブの検証（S6・S7・S8 の図版）

発表者からのレンダリング指摘4件（帰属表示が島を覆う／S8 のラベルが枠外へ溢れる／
3図版で島の形が違う／S7 のパネル幅が不揃い）への対応を検証した。**投影文字列・
スピーカーノート・主張境界は変更していない。**

### 9.1 共有した地図の描画範囲

| 項目 | 値 |
|---|---|
| 陸域 bbox（実測） | 133.51616 / 34.36661 / 133.56019 / 34.40224（CARTO Positron z14 を陸 = 輝度241・海 = 輝度216 で二値化し、島内の点 (133.5369, 34.3900) を含む連結成分の範囲として実測） |
| 採用した描画範囲 `MAP_BBOX` | **133.5135 / 34.3645 / 133.5630 / 34.4045**（P6・S7パネル(c)・P8 の3図版で同一） |
| 四辺の余白 | west 0.00266° / south 0.00211° / east 0.00281° / north 0.00226°（約 230〜260 m） |
| 従来の範囲 | 133.514 / 34.367 / 133.562 / 34.402（陸域 bbox との差が 20〜40 m の**余白ゼロ**。海岸線が四辺すべてで枠に接し、島が切れて見えていた） |
| 縦横比 | 1.0213（Web Mercator。S7 の3パネル共通比としても使う） |
| 背景地図ラスタ | 同じ範囲で取り直した（z17、4,614 × 4,518 px、342タイル、1.00 MB） |

`KITAGI_BBOX = [133.515, 34.350, 133.570, 34.400]`（解析側の範囲）は採らなかった。
南に約 1.8 km の空虚な海が入り、かつ島の北端（34.40224）を切り落とすためである。
描画範囲が実測 bbox を余白つきで含むことは `assert_map_bbox_frames_island()` が機械検査する。

### 9.2 図中文字の実効サイズ（生成済みPPTXの `shape.width` から復元。現行値）
...
| `poster_f1_study_area.png` / S2 | パネルタイトル・島名ラベル | 16 / 16 / 15 | 7.85 | 8.16 | 1.040 | 16.6 / 16.6 / **15.6** | 288 |
| `p05_index_panels.png` / S5 | パネルタイトル・カラーバー目盛 | 18 | 5.67 | 5.04 | 0.889 | **16.0** | 450 |
| `p06_clusters_map.png` / S6 | ゾーンラベル・タイトル・スケールバー | 32 | 8.21 | 4.23 | 0.516 | **16.5** | 388 |
| `p06_clusters_map.png` / S6 | 背景地図の帰属表示（フッター階層） | 22 | 8.21 | 4.23 | 0.516 | 11.4 | 388 |
| `p07_three_scales.png` / S7 | パネルキャプション・スケールバー | 21 | 9.13 | 8.92 | 0.977 | **20.5** | 246 |
| `p07_three_scales.png` / S7 | 背景地図の帰属表示（フッター階層） | 11.5 | 9.13 | 8.92 | 0.977 | 11.2 | 246 |
| `p08_visit_anchors_map.png` / S8 | 訪問番号・対応表・タイトル・凡例・スケールバー | 32 | 8.26 | 4.16 | 0.504 | **16.1** | 397 |
| `p08_visit_anchors_map.png` / S8 | 背景地図の帰属表示（フッター階層） | 22 | 8.26 | 4.16 | 0.504 | 11.1 | 397 |
| `p12_loop_diagram.png` / S12 | ボックスタイトル・キャプション | 27 | 12.35 | 7.55 | 0.612 | **16.5** | 327 |

太字は 15 pt 下限の検査対象（`NATIVE_FONT_SIZES` に宣言した主題ラベル）。**すべて下限以上**
（最小 15.6 pt）。帰属表示のみフッター階層（11.1〜11.4 pt）で意図的に検査対象外。
実効dpi はすべて下限 200 dpi 以上（最小 246 dpi）。

`p08_visit_anchors_map.png` は新たに「対応表（anchor key）」を `NATIVE_FONT_SIZES` に宣言し、
「訪問ラベル（visit label）」を「訪問番号（visit number）」に改名した（検査項目は1つ増えた。
検査を緩めた箇所は無い）。S8 の実効ptが 16.6 → 16.1 pt へ下がったのは、地図の下に対応表2行を
足したぶん図版の実寸高さが伸びたためである（S8 は高さ拘束）。同じ理由で地図そのものは
スライド上で 3.80 in 幅 → 2.73 in 幅に小さくなった。これは「6行の文字（タイトル2行・対応表2行・
凡例2行）を 15 pt 以上で置く」という制約から幾何学的に決まる上限に近い値である
（実効15 ptちょうどの理論上限は 2.79 in 幅）。

### 9.3 グレースケール確認（背景地図と主題データの分離）

`Pillow` の `.convert("L")` で P6・P7・P8 をグレースケール化して読んだ。

| 図版 | 最小輝度 | 1パーセンタイル | 中央値 |
|---|---:|---:|---:|
| `p06_clusters_map.png` | 39 | 43 | 235 |
| `p07_three_scales.png` | 0 | 38 | 224 |
| `p08_visit_anchors_map.png` | 0 | 43 | 255 |

最小値0はパネル(a)(b) の写真の暗部と訪問地点の黒い三角マーカーである。減光後（`KEEP = 0.80`）の
背景地図そのものの最小輝度は **222**、検出ポリゴンの色 `COL_WATER = #0d47a1` のグレースケール
輝度は **64**（透過0.95で背景と合成しても約71）で、**158階調**離れている（是正前と同一）。
背景地図内の海陸差も約 20 階調残っており、海岸線が判別できる。背景地図を薄くしたり
主題データを濃くしたりする追加調整は不要だった。

### 9.4 レンダリング目視（S6・S7・S8）

```bash
soffice --headless --convert-to pdf --outdir /tmp/polish docs/presentations/exp002_kitagi_foss4g2026_presentation.pptx
pdftoppm -r 90 -png -f 6 -l 8 /tmp/polish/exp002_kitagi_foss4g2026_presentation.pdf /tmp/polish/s
```

書き出した3枚を画像として読み、以下を確認した。

- **S6**: 帰属表示は地図枠の**下**にあり島に重ならない。ゾーンラベル4枚は枠内に収まり互いに重ならない。
  島は四辺に余白を持って全体が入る。図版タイトルは2行で右端に余裕がある
- **S7**: 3パネルの表示寸法が同一（実測 assert で固定）。パネル(c) の島の輪郭は S6・S8 と同じ。
  帰属表示はパネル(c) の枠の下。キャプション3本はいずれも自列幅に収まる
- **S8**: 地図面には番号 1〜4 だけがあり、いずれも自分の地点のすぐ隣（リーダー線は 380〜520 m、
  地図上で 0.15〜0.20 in）に置かれている。番号どうし・検出ポリゴン・スケールバー・枠との
  重なりは無い（3条件すべてを生成時に assert 検査）。地名は語中で折れておらず、
  地図の下の対応表に2行×2列で収まる。島の輪郭は S6・S7(c) と同じ

3枚を並べて、島の輪郭が**同じ形**であることを確認した（3図版が同一の `MAP_BBOX` を使う）。

### 9.5 決定性・ネットワーク非依存・機械検査

- 図版生成 → 12枚版 → 再訪なし版 を2回連続で実行し、成果物のSHA256が一致することと
  `git status` に想定外の差分が出ないことを確認した。`_normalize_zip_timestamps()` は未変更
- `https_proxy` / `http_proxy` を無効アドレス（`127.0.0.1:9`）へ向け `uv run --offline` で
  図版生成を実行 → 正常完走。追跡ラスタを退避すると `FileNotFoundError` で
  `--fetch-basemap` の再実行を促して停止することも実測した
- 検査3本すべて合格: 発表 `OK: 1066 checks passed`（是正前 1064 → +2。両デッキで
  `p08_visit_anchors_map.png` の宣言要素が1つ増えたぶん。**緩めた検査は無い**）、
  口頭資料 `OK: 212 checks passed`、ポスター `全チェック合格`
- 図版生成側に追加した機械検査: `assert_credit_outside_map()`（帰属表示が地図軸の外にある）、
  `assert_map_bbox_frames_island()`（描画範囲が陸域 bbox を余白つきで含む）、
  `assert_labels_clear_of_polygons()`（注記が検出ポリゴンを覆わない）、
  `assert_key_below_map()`（対応表が地図の下にあり互いに重ならない）、
  P7 の3パネル同寸 assert、P8 の実寸高さ上限 assert
- ファイルサイズ: 追跡ラスタ 0.95 → 1.00 MB、`p06` 0.48 → 0.40 MB、`p07` 1.23 → 1.28 MB、
  `p08` 0.41 → 0.34 MB、PPTX 各 8.94/8.92 → 8.85/8.83 MB
