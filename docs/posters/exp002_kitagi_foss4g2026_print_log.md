...
3. **色校正の費用・本番納期への影響**（申込方法は公開済みのため上記 Proofing option を参照）
...
**ポスター印刷の発注は中止する。** 主催者からのメール3通（採択通知・スケジュール通知2通）により、本採択が**口頭発表（General Track、9/2 13:30–14:00、Himawari、英語30分）**であり、ポスター発表ではないことが確定した。根拠は `exp002_kitagi_foss4g2026_proposal.md` の「発表形式（2026-08-23 確定）」を参照。

以下の入稿仕様の確認記録は、将来ポスターを印刷する機会のために残す。今回は発注しないため、未解決事項（PDFバージョン・Type 3・色校正）の確認も不要。
