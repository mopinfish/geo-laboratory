...
8) 〔2026年8月24日追記〕本文「２．145個の水域」の最大水域の同定は、その後の訂正により誤りである。OpenStreetMap との照合（`docs/results/exp002/exp002_osm_comparison.md`）にもとづき、報告書（`docs/reports/exp002_kitagi_quarry_water_detection_report.md`）§5.3 を2026年8月23日に訂正した。訂正後の同定は、**最大水域である島北部の7,826m²が OpenStreetMap に「今岡石材丁場跡（北木の桂林）」として登録されている地物と重なる**位置にあり、島南東部の6,521m²のポリゴンの周囲約700m以内には該当する OpenStreetMap 地物が存在しない、というものである。本文の当該2文は投稿時点の記述としてそのまま残している（既に投稿済みの原稿であり、本文を後から書き換えると記録としての履歴が失われるため）。
