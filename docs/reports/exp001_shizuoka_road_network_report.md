...
このことは、市域全体の回遊性指標の解釈において、都市規模（人口や都心部の発達度）よりも市域の地理的範囲と地形条件が支配的であることを示唆する。回遊性の都市間比較には、同程度のスケールのエリア（例えば駅周辺圏）での比較が不可欠である（5-C節で検討）。
...
近接中心性でも焼津市が最高値を示したことから、コンパクトな市域では各ノードからネットワーク全体への到達性が高く、アクセス性の面で優位であることが確認された。ただし、この結果は市域の範囲設定に強く依存するため、絶対値の比較よりも沿岸部/内陸部の対比（5-B節）や同一スケールの駅周辺比較（5-C節）による相対的な評価がより有意義である。
...
この高迂回性の要因として、清水区と駿河区の間に位置する日本平・有度山の丘陵地が挙げられる。沿岸部地図（上図）からも、丘陵地を挟んで道路ネットワークが南北に分断されている様子が読み取れる。防災の観点では、津波発生時に内陸方向への避難経路が丘陵地により制約され、避難距離が直線距離の1.3倍以上に延伸する可能性がある。
...
本節では3つの分析レベル（5-A, 5-B, 5-C）の結果を統合し、仮説4「8指標による多面的評価により都市固有のネットワーク特性を識別できる」を中心に全体的な解釈を行う。
...
本研究は静岡県3都市の沿岸部道路ネットワーク特性を定量的に把握し、以下の具体的知見を提供した。

第一に、仮説1の棄却を通じて、沿岸部の道路構造に対する一般的な想定——沿岸部は回遊性が低く脆弱——が静岡県の3都市には当てはまらないことを実証した。3市すべてにおいて沿岸部の alpha_index が内陸部を上回り、沿岸平野部の市街地が一定の冗長性を持つ道路網を形成していることが確認された。

第二に、静岡市沿岸部の高迂回性（avg_circuity_A = 1.306）を特定し、日本平・有度山の丘陵地による道路ネットワークの分断が避難経路の延伸リスクを生じさせている可能性を指摘した。3市の沿岸部の中で静岡市が最も高い迂回性を示すという知見は、津波避難計画における優先的な検討対象の特定に資する。

第三に、浜松市沿岸部が最も低い迂回性（1.118）と最も高い交差点密度（81.26 /km²）を有し、避難経路の選択肢が多い構造であることを定量的に示した。

### 7.2 観光への貢献

3駅前地区の歩行者ネットワーク回遊性の比較から、以下の実践的知見が得られた。

焼津駅周辺が3駅中最高の回遊性（alpha_index 0.330）を有し、旧漁村集落の路地が歩行回遊の資源として機能していることが定量的に確認された。細街路効果分析（alpha_index +0.013）は、既存の路地・細街路が回遊性向上に寄与する「隠れた資産」であることを示し、これらの保全・活用がまちなか観光の振興に有効であることを示唆する。

浜松駅周辺は交差点密度が最高（195.1 /km²）であるにもかかわらず回遊性が最低（0.247）であり、大街区構造が歩行回遊を抑制していることが明らかになった。歩行者専用通路による大街区の貫通や、既存の細街路の活用（現状では袋路が多い）が回遊性向上の具体的方策として考えられる。

### 7.3 都市計画への貢献

8指標による多面的評価は、3市それぞれに固有のネットワーク特性プロファイル（「コンパクト高回遊型」「広域高迂回型」「近代格子・大街区型」）を識別することに成功した。この「ネットワーク特性の指紋」は、都市の道路構造を多面的に理解し、都市間比較を行うための実用的な枠組みを提供する。劉の3観点に基づく構造化により、個別指標の数値を超えた解釈が可能となり、防災・観光・都市計画の各応用分野への橋渡しが実現した。
...
劉（2016）の4類型を現代都市の市域全体および機能的サブエリア（沿岸部・駅周辺）に適用した結果、類型体系の一般性と限界の双方が明らかになった。焼津駅周辺の細街路効果が類型IV（寺内町）に類似するパターンを示すなど、類型体系が歴史的地区以外にも適用可能な側面が確認された一方、浜松駅周辺の「高交差点密度・低回遊性」という組み合わせは既存の4類型のいずれにも合致しない固有パターンであった。この結果は、劉の類型体系に「近代格子型」等の新類型を追加する方向での理論的精緻化の必要性を示唆するものであり、類型体系の発展に対する具体的な貢献となる。

### 8.2 OSMnx を用いた実践的分析フレームワークの提示

Boeing (2017) の OSMnx と劉 (2016) の3観点・4類型を組み合わせた分析フレームワークを構築し、実際の3都市への適用を通じてその有効性を実証した。沿岸部/内陸部の分割（海岸線データの取得とバッファ処理）、drive/walk の比較による細街路効果の定量化、劉の4類型との照合によるレーダーチャート可視化という一連の分析パイプラインは、日本の任意の都市に対して再現可能な方法論を提供する。分析コードをノートブック形式（`notebooks/exp001_shizuoka_road_network.ipynb`）で整備していることにより、他の研究者や自治体が自地域の道路ネットワーク分析を追試・応用できる。

### 8.3 防災と観光の統合的視点

本研究は防災（津波避難）と観光（歩行回遊）という2つの応用的観点を同一の8指標体系で評価し、同一指標が文脈により異なる含意を持つことを具体的に示した。例えば、alpha_index は防災の観点では「避難経路の冗長性」を、観光の観点では「歩行回遊の多様性」を表す。avg_circuity_A は防災では「避難距離の延伸リスク」を、観光では「目的地への到達効率」を意味する。このように道路ネットワークの構造的特性が複数の応用分野に対して同時に情報を提供できることを実証したことは、道路ネットワーク分析の多目的利用の方法論的基盤を提示するものである。
