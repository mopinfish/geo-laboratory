...
閉鎖後の丁場跡は、切り立った花崗岩の岩壁に囲まれた凹地に雨水や湧水が溜まり、深さ数m〜20m程度の池状の水域を形成している。これらの丁場跡水域は、「北木の桂林」と呼ばれる景観資源や湖上イベント会場として活用される例がある一方、その全容を網羅的に把握した調査は限られている。オープンデータ上にも部分的な記録はあり、OpenStreetMapには対象範囲内に水域・採石場タグの地物が37件（うち名前付き10件、`landuse=quarry` を持つ名前付き地物は7件）登録されている（2026年8月23日時点、5.3節参照）が、島全体の丁場跡水域を網羅したインベントリは公開されていない。丁場跡の空間分布を体系的に記録することは、産業遺産の保全、観光資源の活用、安全管理の各観点から重要である。
...
> **再現性に関する追記（2026-08-14）**: 表4-1の春季結果（島内113ポリゴン）はレポート確定時（2026年3月）の実行の結果であるが、当該実行の設定はリポジトリに保存されていない。現行コミットのノートブックのパイプライン（複合条件）で同一シーン（`S2C_MSIL2A_20250323T014711_R017_T53SLU_20250323T051113`、processing baseline 05.11）を再計算すると島内180ポリゴンとなり、113は未再現である。なお、指数統計（NDWI最大1.000、MNDWI最大0.615）は表4-1と一致しており、入力データ自体は変化していない。夏季結果（表4-2、島内145ポリゴン）は現行パイプラインで完全に再現されることを確認済みである（`scripts/generate_exp002_poster_figures.py` の検証記録を参照）。
...
**仮説3「夏季画像はNDVI植生マスクとの併用で検出精度が向上する」** — **検証できない**。NDVI植生マスクで除外されたピクセルはわずか9であり、水域候補（NDWI超 OR MNDWI超）マスクへの追加効果は限定的であった。これは、水域候補の領域と高NDVI植生域がほとんど重複しないことによる。北木島の植生は水域指数の閾値を超えるほどの水域的反射特性を示さない。ただし、本仮説が主張するのは「検出精度」の向上であり、本研究には正解データも精度指標も存在しないため（5.4節）、候補マスクへの追加効果が9ピクセルと小さかったことをもって精度への効果を支持または棄却することはできない。現地検証データがない限り、精度への効果は評価できない。

> **判定に関する訂正〔2026年8月24日訂正〕**: 本項は当初、仮説3を「棄却される」としていた。これは誤りである。本研究には正解データも精度指標もなく（5.4節参照）、NDVI植生マスクが水域候補マスクに与えた変化（9ピクセル）は候補マスクへの効果であって精度指標ではない。精度が未計測である以上、9ピクセルという小さな変化から精度への効果の有無を推定して仮説を棄却することはできないため、判定を「検証できない」に改めた。候補マスクへの追加効果が限定的であったという観察自体（9ピクセル、水域候補と高NDVI植生域の重複がほとんどないこと）は変更していない。
...
検出水域と既知の丁場位置の対応を確認するため、OpenStreetMap（OSM）に登録済みの水域・採石場地物と照合した（照合日2026年8月23日、記録は `docs/results/exp002/exp002_osm_comparison.md`）。距離は地物形状をEPSG:32653へ投影した形状間距離であり、重なる場合は0mである。バウンディングボックス内のOSM地物は37件（すべて面）で、うち10件が名前付き、7件が `landuse=quarry` を持つ。

**表5-1 名前付きの採石場・丁場跡地物と検出ポリゴンの位置関係**

| OSM 地物 | 最近傍検出との距離 | 重なった検出の面積 |
| --- | ---: | ---: |
| 今岡石材丁場跡（北木の桂林） | 0 m | 7,826 m² |
| 馬越道也石材丁場跡 | 0 m | 7,826 m² |
| 馬越石材工業所丁場跡 | 0 m | 4,013 m² |
| 竹本丁場跡 | 0 m | 4,013 m² |
| 村上石材丁場跡 | 0 m | 2,508 m² |
| 鶴田石材採石場 | 0 m | 100 m² |
| 花本石材採石場 | 0 m | 100 m² |

名前付きの `landuse=quarry` 地物**7件すべて**が検出ポリゴンと重なった。OSM地物37件のうち21件が検出ポリゴンと重なり、34件は100m以内に検出ポリゴンが存在した。一方、検出145件のうちOSM地物が100m以内にあるものは65件で、**80件は近傍にOSM地物を持たない**。100mは探索的な近傍判定の閾値であり、閾値による感度は照合記録に併記した。

「北木の桂林」と呼ばれる旧今岡石材の丁場跡について、OSMの位置情報は島北部を示し、当該地物は検出最大水域（7,826m²）と重なる。一方、南東部の6,521m²の水域近傍にはOSM地物がない。したがって、南東部の水域を「北木の桂林」とする対応づけは支持されず、**検出最大水域が「北木の桂林」の有力な現地確認候補**である。ただしOSMの名称・位置もコミュニティ寄稿であり、確定には現地での位置確認（GPS・撮影方向・撮影時刻の記録）が必要である。

なお、名前付きの `water=reservoir` 地物（北木北ポンプ室）が100m²の検出ポリゴンから2.7mの距離にある。これは5.4節が挙げる人工ため池・貯水施設の交絡例にあたるが、当該検出を誤検出と断定するものではない。

OSMはコミュニティ寄稿データであり正解データではない。上記の重なりは独立した傍証であって精度検証ではなく、近傍にOSM地物を持たない80件は「OSM未登録」か「誤検出」のいずれかであり、状態は未決着である。
...
6. **OSM照合の位置づけ**: 5.3節のOSM照合は、コミュニティ寄稿データとの一致を見たものであり精度検証ではない。OSMの網羅性・位置精度は保証されておらず、適合率・再現率の算出には現地検証データが必要である
7. **春季結果の再現性**: 春季の島内113ポリゴンは過去の確定実行結果であり、当該実行の設定が保存されていないため、現行コミットのパイプラインでは再現できない（4.1節の追記を参照）。夏季結果（145ポリゴン）は再現可能である
...
- Du, Y., Zhang, Y., Ling, F., Wang, Q., Li, W. and Li, X. (2016) Water bodies' mapping from Sentinel-2 imagery with Modified Normalized Difference Water Index at 10-m spatial resolution produced by sharpening the SWIR band. _Remote Sensing_, **8**(4), 354. — 複数水域指数の組み合わせによる小規模水域検出の先行研究
- McFeeters, S. K. (1996) The use of the Normalized Difference Water Index (NDWI) in the delineation of open water features. _International Journal of Remote Sensing_, **17**(7), 1425-1432. — NDWI の提唱論文
- Xu, H. (2006) Modification of normalised difference water index (NDWI) to enhance open water features in remotely sensed imagery. _International Journal of Remote Sensing_, **27**(14), 3025-3033. — MNDWI の提唱論文。裸地・建造物と水域の分離改善
