...
検出水域と既知の丁場位置の対応を確認するため、OpenStreetMap（OSM）に登録済みの水域・採石場地物と照合した（照合日2026年8月23日、記録は `docs/results/exp002/exp002_osm_comparison.md`）。バウンディングボックス内の OSM 地物は37件で、うち10件が名前付きであった。

**表5-1 名前付きOSM地物と最近傍検出ポリゴンの距離（抜粋）**

| OSM 地物 | 位置 | 最近傍検出との距離 | 該当ポリゴン面積 |
| --- | --- | ---: | ---: |
| 花本石材採石場 | 34.3874°N, 133.5202°E | 3 m | 502 m² |
| 鶴田石材採石場 | 34.3875°N, 133.5314°E | 6 m | 4,715 m² |
| 村上石材丁場跡 | 34.3904°N, 133.5216°E | 9 m | 2,508 m² |
| 馬越石材工業所丁場跡 | 34.3902°N, 133.5322°E | 17 m | 4,013 m² |
| 馬越道也石材丁場跡 | 34.3912°N, 133.5335°E | 22 m | 7,826 m² |
| 今岡石材丁場跡（北木の桂林） | 34.3912°N, 133.5329°E | 37 m | 7,826 m² |

名前付きの丁場跡・採石場はいずれも検出ポリゴンの近傍（3〜39 m）に位置し、既知の丁場と検出水域が空間的に対応することが確認された。OSM地物37件のうち30件は検出ポリゴンが100 m以内に存在した一方、検出145件のうちOSM地物が100 m以内にあるものは54件であり、91件はOSMに対応地物を持たない。

「北木の桂林」と呼ばれる旧今岡石材の丁場跡について、OSMの位置情報は島北部（34.3912°N, 133.5329°E）を示し、これは検出最大水域（7,826 m²、34.3911°N, 133.5333°E）と37 mの距離で一致する。すなわち**検出最大水域が「北木の桂林」に相当すると考えられる**。一方、南東部の6,521 m²の水域には対応するOSM地物がなく（最近傍の地物まで796 m）、同地を「北木の桂林」とする対応づけは支持されない。

なお、OSMはコミュニティ寄稿データであり正解データではない。上記の一致は独立した傍証であって精度検証ではなく、OSMに対応地物を持たない91件は「OSM未登録」か「誤検出」のいずれかである。確定には現地での位置確認が必要である。
