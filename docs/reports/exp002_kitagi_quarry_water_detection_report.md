...
> **再現性に関する追記（2026-08-14）**: 表4-1の春季結果（島内113ポリゴン）は2026年3月21日の確定実行の結果であるが、当該実行の設定はリポジトリに保存されていない。現行コミットのノートブックのパイプライン（複合条件）で同一シーン（`S2C_MSIL2A_20250323T014711_R017_T53SLU_20250323T051113`、processing baseline 05.11）を再計算すると島内180ポリゴンとなり、113は未再現である。なお、指数統計（NDWI最大1.000、MNDWI最大0.615）は表4-1と一致しており、入力データ自体は変化していない。夏季結果（表4-2、島内145ポリゴン）は現行パイプラインで完全に再現されることを確認済みである（`scripts/generate_exp002_poster_figures.py` の検証記録を参照）。
